...
// GraphEngine implements kgtypes.Graph by orchestrating Store + Extractor + Embedder.
type GraphEngine struct {
	store     kgtypes.Store
	extractor kgtypes.Extractor
	embedder  kgtypes.Embedder
	ontology  *kgtypes.Ontology
	logger    *slog.Logger
}
...
func (e *GraphEngine) ApplyOntology(_ context.Context, ont *kgtypes.Ontology) error {
	e.ontology = ont
	return nil
}

// IngestEpisode extracts entities/relations from text, deduplicates, and stores them.
func (e *GraphEngine) IngestEpisode(ctx context.Context, input *kgtypes.EpisodeInput) (*kgtypes.IngestResult, error) {
	if e.extractor == nil {
		return nil, kgtypes.ErrNoExtractor
	}
	if e.store == nil {
		return nil, kgtypes.ErrStoreNotReady
	}
	...
	responseEntities := make([]kgtypes.Entity, 0, len(extractedEntities))
	...
	for _, ent := range extractedEntities {
		...
		responseEntities = append(responseEntities, kgtypes.Entity{
	...
	now := time.Now()
	responseRelations := make([]kgtypes.Relation, 0, len(extractedRelations))
	...
	for _, rel := range extractedRelations {
		...
		relUUID, err := e.store.CreateRelation(ctx, &kgtypes.RelationInput{
			SourceUUID: srcUUID,
			TargetUUID: tgtUUID,
			Type:       rel.Type,
			Fact:       rel.Fact,
		...
		responseRelations = append(responseRelations, kgtypes.Relation{
			UUID:       relUUID,
			SourceUUID: srcUUID,
			TargetUUID: tgtUUID,
			Type:       rel.Type,
...
func (e *GraphEngine) deduplicateAndUpsertEntity(ctx context.Context, ent *kgtypes.ExtractedEntity) (string, error) {
	// Generate embedding
	...
	if err == nil {
		for _, candidate := range candidates {
			if fuzzy.IsFuzzyMatch(ent.Name, candidate.Name, FuzzyMatchThreshold) {
				...
				merged := &kgtypes.ExtractedEntity{
...
func (e *GraphEngine) GetEntity(ctx context.Context, id string) (*kgtypes.Entity, error) {
	return e.store.GetEntity(ctx, id)
}

// SearchFacts combines vector and BM25 search using Reciprocal Rank Fusion.
func (e *GraphEngine) SearchFacts(ctx context.Context, query string, opts ...kgtypes.SearchOption) (*kgtypes.SearchFactsResult, error) {
	o := &kgtypes.SearchOptions{}
	...
	var vectorResults []kgtypes.ScoredFact
	var bm25Results []kgtypes.ScoredFact
...
func reciprocalRankFusion(listA, listB []kgtypes.ScoredFact, limit int) []kgtypes.Fact {
	scores := make(map[string]float64)
	factMap := make(map[string]kgtypes.Fact)
...
func (e *GraphEngine) GetGraph(ctx context.Context, limit int64) (*kgtypes.GraphData, error) {
	return e.store.GetGraph(ctx, limit)
}

// GetNode returns a node with its neighbors and edges at the requested depth.
func (e *GraphEngine) GetNode(ctx context.Context, id string, depth int) (*kgtypes.NodeDetail, error) {
	return e.store.GetNode(ctx, id, depth)
}

// GetFactProvenance returns the episodes that sourced a given fact.
func (e *GraphEngine) GetFactProvenance(ctx context.Context, factUUID string) ([]kgtypes.Episode, error) {
