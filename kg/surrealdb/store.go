...
// Store implements kgtypes.Store using SurrealDB.
type Store struct {
	db     *surrealdbgo.DB
	logger *slog.Logger
}
...
func (s *Store) UpsertEntity(ctx context.Context, entity *kgtypes.ExtractedEntity, embedding []float32) (string, error) {
...
func (s *Store) GetEntity(ctx context.Context, id string) (*kgtypes.Entity, error) {
	...
	if err != nil || result == nil || len(*result) == 0 || len((*result)[0].Result) == 0 {
		return nil, fmt.Errorf("%w: %s", kgtypes.ErrNodeNotFound, id)
	}

	rec := (*result)[0].Result[0]
	return &kgtypes.Entity{UUID: rec.UUID, Name: rec.Name, Type: rec.Type, Summary: rec.Summary}, nil
}

// FindEntitiesByNameType finds entities with exact name+type match.
func (s *Store) FindEntitiesByNameType(ctx context.Context, name, entityType string) ([]kgtypes.Entity, error) {
	result, err := surrealdbgo.Query[[]nodeRecord](ctx, s.db,
...
func (s *Store) FindEntitiesByFuzzyName(ctx context.Context, name string, limit int) ([]kgtypes.Entity, error) {
	if limit <= 0 {
		limit = 10
	}
	result, err := surrealdbgo.Query[[]nodeRecord](ctx, s.db,
...
func (s *Store) CreateRelation(ctx context.Context, rel *kgtypes.RelationInput) (string, error) {
...
func (s *Store) FindRelationsBetweenEntities(ctx context.Context, srcUUID, tgtUUID string) ([]kgtypes.Relation, error) {
	...
	var rels []kgtypes.Relation
	for _, rec := range (*result)[0].Result {
		rels = append(rels, kgtypes.Relation{
			UUID:       rec.UUID,
...
func (s *Store) CreateEpisode(ctx context.Context, input *kgtypes.EpisodeInput, entityUUIDs []string) (string, error) {
...
func (s *Store) SearchByEmbedding(ctx context.Context, embedding []float32, opts *kgtypes.SearchOptions) ([]kgtypes.ScoredFact, error) {
	if opts == nil {
		opts = &kgtypes.SearchOptions{}
	}
...
func (s *Store) SearchByText(ctx context.Context, query string, opts *kgtypes.SearchOptions) ([]kgtypes.ScoredFact, error) {
	if opts == nil {
		opts = &kgtypes.SearchOptions{}
	}
...
func (s *Store) GetGraph(ctx context.Context, limit int64) (*kgtypes.GraphData, error) {
	...
	nodeMap := make(map[string]kgtypes.GraphNode)
	var edges []kgtypes.GraphEdge
	...
	nodes := make([]kgtypes.GraphNode, 0, len(nodeMap))
	for _, n := range nodeMap {
		nodes = append(nodes, n)
	}

	return &kgtypes.GraphData{Nodes: nodes, Edges: edges}, nil
}

// GetNode returns a node with its neighbors and edges.
// depth controls how many hops to traverse (1 = immediate neighbors).
func (s *Store) GetNode(ctx context.Context, id string, depth int) (*kgtypes.NodeDetail, error) {
	if depth < 1 {
		depth = 1
	}
	...
	return &kgtypes.NodeDetail{Node: rootNode, Neighbors: allNeighbors, Edges: allEdges}, nil
}

// getNeighbors returns the immediate neighbors and edges for a node UUID.
func (s *Store) getNeighbors(ctx context.Context, nodeUUID string) ([]kgtypes.GraphNode, []kgtypes.GraphEdge, error) {
	...
	var neighbors []kgtypes.GraphNode
	var edges []kgtypes.GraphEdge
	...
	if relResult != nil && len(*relResult) > 0 {
		for _, row := range (*relResult)[0].Result {
			...
			if !seen[row.NUUID] {
				seen[row.NUUID] = true
				neighbors = append(neighbors, kgtypes.GraphNode{
...
func (s *Store) GetFactProvenance(ctx context.Context, factUUID string) ([]kgtypes.Episode, error) {
	...
	var episodes []kgtypes.Episode
	for _, rec := range (*result)[0].Result {
		episodes = append(episodes, kgtypes.Episode{
			UUID:      rec.UUID,
			Name:      rec.Name,
			Body:      rec.Body,
			Source:    rec.Source,
...
func (s *Store) entityRecordID(ctx context.Context, entityUUID string) (*models.RecordID, error) {
	...
	if err != nil || result == nil || len(*result) == 0 || len((*result)[0].Result) == 0 {
		return nil, fmt.Errorf("%w: %s", kgtypes.ErrNodeNotFound, entityUUID)
	}
...
func (s *Store) factRowsToScoredFacts(rows []factRow) []kgtypes.ScoredFact {
	facts := make([]kgtypes.ScoredFact, 0, len(rows))
	seen := make(map[string]bool)
	for _, row := range rows {
		...
		seen[row.RUUID] = true
		facts = append(facts, kgtypes.ScoredFact{
			Fact: kgtypes.Fact{
				UUID:     row.RUUID,
				Name:     row.RType,
				FactText: row.RFact,
				SourceNode: kgtypes.Entity{
					UUID: row.SrcUUID, Name: row.SrcName, Type: row.SrcType, Summary: row.SrcSummary,
				},
				TargetNode: kgtypes.Entity{
...
func nodeRecordsToEntities(result *[]surrealdbgo.QueryResult[[]nodeRecord]) []kgtypes.Entity {
	if result == nil || len(*result) == 0 {
		return nil
	}
	var entities []kgtypes.Entity
	for _, rec := range (*result)[0].Result {
		entities = append(entities, kgtypes.Entity{
